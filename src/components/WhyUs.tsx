...
export default function WhyUs({ language }: WhyUsProps) {
  ...
  return (
    <section id="why-us" className="py-24 bg-stone-50" ref={sectionRef}>
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          {/* ZMIANA: Zmniejszono odstępy na mobile/tablet (space-y-12) i zachowano na desktop (lg:space-y-24) */}
            ...
            {content.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  data-index={index}
                  // ZMIANA: Usunięto lg:flex-row, lg:flex-row-reverse - będą one dodane niżej dla uproszczenia
                  className={`journey-item relative flex flex-col lg:flex-row items-center transition-all duration-700 ${
                    isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
                >
                  {/* ZMIANA: Cały kafelek (div) jest teraz osobnym blokiem dla łatwiejszego zarządzania kierunkiem na desktopie */}
                  <div className={`w-full lg:flex-1 ${isLeft ? 'lg:order-1 lg:pr-16' : 'lg:order-2 lg:pl-16'}`}>
                    <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden">
                      {/* ZMIANA: Wyższe zdjęcie na mobile/tablet (h-64), zachowane na desktop (lg:h-52) */}
                      <img src={item.image} alt={item.title} className="w-full h-64 lg:h-52 object-cover" />
                      <div className="p-8">
